...
import {verifyStakeEvent,verifyWithdrawEvent,verifySetRewardTokenFundsEvent} from './contract/verify-staking-events'
...
describe('Staking', function()  {
    ...
    beforeEach(async function () {
        ...
        this.lockUpStartTime = (await  this.provider.getBlock(blockNumber)).timestamp;
        ...
        this.stakingContract = await deployContract<Staking>('Staking',
    ...
    it('Admin should deposit funds for staking rewards', async function() {
        ...
        this.rewardToken.connect(this.admin).approve(this.stakingContract.address,depositAmount);
        const depositReceipt = await successfulTransaction(
            this.stakingContract.connect(this.admin).setRewardTokenFunds(depositAmount)
        )

        const rewardTokenBalance : BN = await this.rewardToken.balanceOf(this.stakingContract.address);
    ...
    it('User should stake within lockup period', async function() {
        ...
        const stakeReceipt = await successfulTransaction(
            this.stakingContract.connect(this.user1).stake({value: stakingAmount})
        );
        const user1Stakes = await this.stakingContract.balances(this.user1.address);
    ...
    it('User should withdraw after lockupEndTime', async function() {
        ...
        this.rewardToken.connect(this.admin).approve(this.stakingContract.address, testWithdrawAmount);
        this.stakingContract.connect(this.admin).setRewardTokenFunds(testWithdrawAmount);
        this.stakingContract.connect(this.user1).stake({value: testWithdrawAmount})

        // Withdraw
        await this.stakingContract.updateLockupEndTime(this.lockUpStartTime);
        const withdrawReceipt = await successfulTransaction(
            this.stakingContract.connect(this.user1).withdraw(this.withdrawAmount)
        );
        const user1RewardBalance : BN = await this.rewardToken.balanceOf(this.user1.address);
        verifyWithdrawEvent(withdrawReceipt, (this.withdrawAmount),user1RewardBalance)
    });
});
